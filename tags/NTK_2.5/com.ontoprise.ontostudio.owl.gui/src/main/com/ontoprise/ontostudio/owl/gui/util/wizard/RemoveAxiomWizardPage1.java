    ...
    private String getInfoString() throws NeOnCoreException {
        ...
        if (_sourceEntityUri.equals("") && _targetEntityUri.equals("")) { //$NON-NLS-1$ //$NON-NLS-2$
            ...
            if (entities.size() > 1) {
                ...
                info.append(_namespaces.abbreviateAsNamespace(entities.isEmpty()?"":entities.get(0).getIRI().toString())); //$NON-NLS-1$
                info.append("]"); //$NON-NLS-1$
            }
        } else {
            info.append(Messages.RemoveAxiomWizardPage1_6);
            OWLClassExpression sourceDesc = OWLUtilities.description(_sourceEntityUri);
            OWLClassExpression targetDesc = OWLUtilities.description(_targetEntityUri);
