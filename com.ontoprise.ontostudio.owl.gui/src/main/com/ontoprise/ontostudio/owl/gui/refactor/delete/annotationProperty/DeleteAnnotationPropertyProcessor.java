...
public class DeleteAnnotationPropertyProcessor extends DeleteProcessor {
	public static final String IDENTIFIER = "com.ontoprise.ontostudio.owl.gui.refactor.delete.annotationProperty.DeleteAnnotationPropertyProcessor"; //$NON-NLS-1$

	private AnnotationPropertyTreeElement[] _elements;
	private AnnotationPropertyTreeElement[] _parents;

	public DeleteAnnotationPropertyProcessor(AnnotationPropertyTreeElement[] elements, AnnotationPropertyTreeElement[] parents) {
		_elements = elements;
		_parents = parents;
	}
	...
	public Change createChange(IProgressMonitor pm) throws CoreException,
			OperationCanceledException {
		return new AnnotationPropertyDeleteChange(
				_elements, 
				_parents);				
	}
	...
	public RefactoringParticipant[] loadParticipants(RefactoringStatus status,
		...
		for (int i=0; i<elements.length; i++) {
			result.addAll(Arrays.asList(ParticipantManager.loadDeleteParticipants(status, 
					this, elements[i], 
					new DeleteArguments(), natures, sharedParticipants)));
		}
