...
public class DatatypeFolderProvider extends DefaultTreeDataProvider {
    private DatatypeFolderTreeElement _folder;
    ...
    public ITreeElement[] getElements(ITreeElement parentElement, int topIndex, int amount) {
        ...
        _folder = new DatatypeFolderTreeElement(((IProjectElement) parentElement).getProjectName(), ((IOntologyElement) parentElement).getOntologyUri(), this);
    ...
    public TreeElementPath[] getPathElements(ITreeElement element) {
        if (element instanceof DatatypeTreeElement) {
            ...
            DatatypeTreeElement clazzTreeElement = ((DatatypeTreeElement) element);
            DatatypeFolderTreeElement clazzFolderTreeElement = new DatatypeFolderTreeElement(clazzTreeElement.getProjectName(), clazzTreeElement.getOntologyUri(), this);
            path.append(clazzFolderTreeElement);
            return new TreeElementPath[] {path};
        } else if (element instanceof DatatypeFolderTreeElement) {
            TreeElementPath path = new TreeElementPath();
            path.append(element);
            return new TreeElementPath[] {path};
        }
